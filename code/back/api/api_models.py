from pydantic import BaseModel, NonNegativeFloat, NonNegativeInt, PositiveInt, model_validator
...
class PredictionRequest2(BaseModel):
    ...
    def check_lengths_match_planning_horizon(self) -> Self:
        assert (
            len(self.purchase_costs_pln_per_kg_daily)
            == self.planning_horizon_days
            == len(self.num_conferences_daily)
            == len(self.num_workers_daily)
            ...
            "examples": [
                {
                    ...
                    "num_conferences_daily": [1, 0, 3, 7, 0, 0, 0],
                    "num_workers_daily": [50, 90, 60, 50, 31, 15, 15],
